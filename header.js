  ...
  function initTopInfoBar() {
    try {
      ...
      if (header) {
        ...
        // Captura o valor BASE já existente na variável ANTES de qualquer alteração
        const existingHeight = parseInt(
          getComputedStyle(document.documentElement).getPropertyValue('--notification-bar-height')
        ) || 0;

        // CSS usa variável CSS dinâmica
        ...
        // Faz refresh do plugin sticky dos filtros após a variável ser atualizada
        // O próprio site usa este padrão noutros scripts (bannerWelcomeGift, etc.)
        function refreshFilters() {
          if (typeof fixedFilters !== 'undefined') {
            if (typeof $.fn.hcSticky === 'function') {
              fixedFilters.hcSticky('refresh');
            } else if (typeof $.fn.stickinparent === 'function') {
        ...
        // Atualiza a variável CSS somando o valor base existente + altura da barra * 2
