(function () {
  ...
  function injectMiniCartPromo(products) {
    ...
    globalObserver.disconnect(); // Desconectar temporariamente para evitar loops
    ...
    // Injeta logo a seguir aos produtos do mini-cart
    $productsList.after(cardHtml);

    // Reatar observação global
    globalObserver.observe(document.body, { childList: true, subtree: true });
    isInjectingMiniCart = false;
  }

  // MutationObserver Global no body para detetar re-renderizações e modificações
    ...
    setTimeout(function() {
      ...
      if ($miniCart.length > 0 && $miniCart.is(':visible')) {
        ...
        if ($productsList.length > 0) {
          if ($('#mm-minicart-promo-card').length === 0) {
  ...
  function initPromo() {
    ...
    // Iniciar MutationObserver global no body
    globalObserver.observe(document.body, { childList: true, subtree: true });
  }
