(function () {
  ...
  const translations = {
    ...
    es: {
      ...
      subtitle1: "Añade <strong>1 artigo más de Havaianas</strong> al carrito para desbloquear esta oferta.",
  ...
  // CSS DO CARD (Checkout, Mini-Cart e Painel de Recomendação)
  ...
  function getHavaianasCount() {
    ...
    // Ler do Mini-cart DOM ou do Painel de Recomendação (se algum estiver visível)
  ...
  function initPromo() {
    ...
    // Injeção imediata na abertura manual (consola) se já estiverem visíveis
    const $miniCart = $('#rdc-mini-cart');
    if ($miniCart.length > 0 && $miniCart.is(':visible')) {
      injectPromoCard($miniCart, '#mm-minicart-promo-card');
    }
