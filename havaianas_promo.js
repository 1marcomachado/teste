(function () {
  ...
  if ($('#mm-promo-card-style').length === 0) {
    $('head').append(`
      <style id="mm-promo-card-style">
        ...
        /* Botão compacto para caber perfeitamente no telemóvel sem overflow */
  ...
  // MutationObserver Local focado que escuta apenas modificações nos popups (zero overhead no body)
  ...
  const bodyObserver = new MutationObserver(function () {
    ...
    if (miniCartEl && !miniCartObserved) {
      localObserver.observe(miniCartEl, { childList: true, subtree: true });
      miniCartObserved = true;
    }
    if (recCartEl && !recCartObserved) {
      localObserver.observe(recCartEl, { childList: true, subtree: true });
      recCartObserved = true;
    }

    // Se ambos os containers já estão a ser monitorizados localmente, desliga permanentemente o bodyObserver
