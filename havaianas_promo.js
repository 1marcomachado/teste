(function () {
  ...
  console.log("[MM INFO] Inicializando script Havaianas Promo com proteção ativa de loops...");
  ...
  function updatePromoCardProgress($existing) {
    ...
    // Se um dos produtos atualmente sugeridos foi adicionado ao carrinho, baralha e atualiza o DOM interno do card silenciosamente (sem triggers de mutação)
    const cartProductIds = getCartProductIds();
    const hasSuggestedInCart = shuffledProducts.some(p => cartProductIds.includes(p.id));
    
    if (hasSuggestedInCart && !isUnlocked) {
      console.log("[MM INFO] Um dos produtos sugeridos foi detetado no carrinho. Baralhando novamente...");
    ...
    let cardHtml = `
      <div id="${cardId.replace('#', '')}">
        <div class="mm-promo-header ${isUnlocked ? '' : 'has-products'}">
          ...
          <div class="mm-promo-info">
            <div class="mm-promo-title">${isUnlocked ? t.unlockedTitle : t.title}</div>
            <div class="mm-promo-subtitle">dots ${subtitleText}</div>
            <div class="mm-progress-container"><div class="mm-progress-bar" style="width: ${percentage}%;"></div></div>
          </div>
        </div>
    `;
  ...
  // Contador de execuções para parar loops inesperados em browser e evitar empancar de vez!
  let totalObserverFires = 0;
  ...
  const localObserver = new MutationObserver(function (mutations) {
    totalObserverFires++;
    console.log("[MM DEBUG] localObserver disparou #" + totalObserverFires + ". Mutações detetadas:", mutations.length);
    
    // Válvula de segurança: se disparar mais de 30 vezes seguidas no mesmo bloco, desliga temporariamente para evitar empancar
    if (totalObserverFires > 30) {
      console.warn("[MM WARNING] O observador disparou mais de 30 vezes seguidas! Auto-terminando para evitar congelamento.");
      localObserver.disconnect();
      setTimeout(function() {
        totalObserverFires = 0;
        reattachLocalObservers();
        console.log("[MM INFO] Observadores re-anexados após arrefecimento de loop.");
      }, 1000);
      return;
    }
    ...
    // Resetar contagem se as mutações assentarem
    setTimeout(function() {
      totalObserverFires = 0;
    }, 200);
  });
