(function () {
  console.log('[PROMO DEBUG] Script inicializado.');
  
  // Proteção contra múltiplas execuções do script
  if (window.__mmHavaianasPromoInit) {
    console.log('[PROMO DEBUG] Script já estava ativo. Abortando execução duplicada.');
    return;
  }
  ...
  console.log('[PROMO DEBUG] Idioma detetado:', lang);
  ...
  // CSS DO CARD (Checkout, Mini-cart e Recomendação) com suporte para swipe nativo via CSS Snap
  if ($('#mm-promo-card-style').length === 0) {
    $('head').append(`
      <style id="mm-promo-card-style">
        ...
        #mm-minicart-promo-card .mm-product-left, #mm-reccart-promo-card .mm-product-left { width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        #mm-minicart-promo-card .mm-product-mid, #mm-reccart-promo-card .mm-product-mid { flex-grow: 1; padding: 0 10px; min-width: 0; }
        #mm-minicart-promo-card .mm-product-brand, #mm-reccart-promo-card .mm-product-brand { font-size: 9px; text-transform: uppercase; color: #888; font-weight: 700; margin-bottom: 2px; }
        ...
        #mm-minicart-promo-card .mm-product-price, #mm-reccart-promo-card .mm-product-price { font-size: 11px; font-weight: 700; color: #111; margin-top: 2px; }
  ...
  // Obter IDs dos produtos no carrinho (DOM-based)
  function getCartProductIds() {
    const ids = [];
    
    // Ler de ambos os popups
  ...
  // Obter número total de Havaianas no carrinho baseando-se EXCLUSIVAMENTE no DOM ativo para total precisão
  function getHavaianasCount() {
    ...
    if ($activePopup.length > 0) {
      $activePopup.find('.rdc-mini-cart-product').each(function () {
    ...
    // Fallback se nenhuns popups estiverem abertos mas o DOM do mini-cart estiver preenchido
  ...
  function updatePromoCardProgress($existing) {
    const count = getHavaianasCount();
    console.log('[PROMO DEBUG] updatePromoCardProgress chamado. Contagem Havaianas:', count);
    let percentage = count === 0 ? 0 : (count === 1 ? 50 : 100);
    let isUnlocked = count >= 2;
    let subtitleText = count === 0 
      ? t.subtitle0
    ...
    const cartProductIds = getCartProductIds();
    console.log('[PROMO DEBUG] IDs no carrinho:', cartProductIds);
    console.log('[PROMO DEBUG] IDs sugeridos atualmente:', shuffledProducts.map(p => p.id));
    
    const hasSuggestedInCart = shuffledProducts.some(p => cartProductIds.includes(p.id));
    if (hasSuggestedInCart && !isUnlocked) {
      console.log('[PROMO DEBUG] Um produto sugerido foi adicionado ao carrinho! Atualizando sugestões.');
      shuffledProducts = getUpdatedSuggestions();
      $existing.remove();
      console.log('[PROMO DEBUG] Card antigo removido para recriação.');
      return;
    }
    ...
    if ($bar.length > 0) {
      ...
      console.log('[PROMO DEBUG] Atualizando largura da barra para:', newWidth);
  ...
  function injectPromoCard($popup, cardId) {
    console.log('[PROMO DEBUG] injectPromoCard invocado para:', cardId);
    if (isInjecting) {
      console.log('[PROMO DEBUG] Já está a ocorrer injeção. Ignorado.');
      return;
    }

    const $productsList = $popup.find('.rdc-mini-cart-product');
    console.log('[PROMO DEBUG] Produtos encontrados na lista do popup:', $productsList.length);
    if ($productsList.length === 0) {
      console.log('[PROMO DEBUG] Zero produtos no popup. Abortando injeção.');
      return;
    }
    ...
    if ($existing.length > 0) {
      console.log('[PROMO DEBUG] Card já existe. Atualizando progresso.');
      updatePromoCardProgress($existing);
      return;
    }

    console.log('[PROMO DEBUG] Card não existe. Iniciando injeção no DOM.');
    isInjecting = true;
    globalObserver.disconnect();

    const count = getHavaianasCount();
    console.log('[PROMO DEBUG] Contagem Havaianas para injeção:', count);
    let percentage = count === 0 ? 0 : (count === 1 ? 50 : 100);
    let isUnlocked = count >= 2;
    let subtitleText = count === 0 
      ? t.subtitle0
    ...
    // Injeta após o último item do produto no cesto lateral ativo
    const $lastProduct = $productsList.last();
    if ($lastProduct.length > 0) {
      console.log('[PROMO DEBUG] A injetar card após o último produto no DOM.');
      $lastProduct.after(cardHtml);
    } else {
      console.log('[PROMO DEBUG] Falha: Não foi encontrado o último produto.');
    }

    globalObserver.observe(document.body, { childList: true, subtree: true });
    isInjecting = false;
    console.log('[PROMO DEBUG] Injeção concluída.');
  }

  // MutationObserver Global no body garante a captura mesmo com injeção dinâmica de controllers do Angular
  const globalObserver = new MutationObserver(function (mutations) {
    console.log('[PROMO DEBUG] Global MutationObserver disparado! Mutações:', mutations.length);
    
    // 1. Painel 1: Mini-cart
    const $miniCart = $('#rdc-mini-cart');
    if ($miniCart.length > 0 && $miniCart.is(':visible')) {
      console.log('[PROMO DEBUG] MiniCart está visível no DOM.');
      injectPromoCard($miniCart, '#mm-minicart-promo-card');
    }

    // 2. Painel 2: Recomendação Add-to-cart (Dinâmico)
    const $recCart = $('#recomendation-cart-right-bar');
    if ($recCart.length > 0 && $recCart.is(':visible')) {
      console.log('[PROMO DEBUG] RecCart está visível no DOM.');
      injectPromoCard($recCart, '#mm-reccart-promo-card');
    }
  });

  function initPromo() {
    console.log('[PROMO DEBUG] initPromo executado.');
    ...
    shuffledProducts = getUpdatedSuggestions();
    console.log('[PROMO DEBUG] Sugestões iniciais baralhadas:', shuffledProducts.map(p => p.name));

    // Se for a página de Checkout, renderiza no checkout
    if (window.location.href.includes('checkout')) {
      console.log('[PROMO DEBUG] Página de checkout detetada. Renderizando card do checkout.');
      renderCheckoutCard(shuffledProducts);
    }

    // Iniciar a observação global do body
    globalObserver.observe(document.body, { childList: true, subtree: true });
    console.log('[PROMO DEBUG] Observador global anexado ao body.');
    
    // Injeção imediata caso os elementos já estejam abertos na consola
    const $miniCart = $('#rdc-mini-cart');
    if ($miniCart.length > 0 && $miniCart.is(':visible')) {
      injectPromoCard($miniCart, '#mm-minicart-promo-card');
    }
