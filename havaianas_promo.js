(function () {
  ...
  if ($('#mm-promo-card-style').length === 0) {
    $('head').append(`
      <style id="mm-promo-card-style">
        ...
        .rdc-wrapper-popup-body {
          display: flex !important;
          flex-direction: column !important;
        }
        
        .rdc-mini-cart-products, .rdc-mini-cart-product {
          order: 1 !important;
        }
        
        #mm-minicart-promo-card, #mm-reccart-promo-card {
          order: 2 !important;
          width: calc(100% - 20px); border: 1px solid #e6e6e6; border-radius: 8px; background: #fff;
          padding: 16px; margin: 10px auto 20px auto; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.01);
          font-family: 'Roboto', 'RobotoCondensed', sans-serif; text-align: left; box-sizing: border-box;
        }
        
        .rdc-recomendation-combine {
          order: 3 !important;
        }
        
        .rdc-wrapper-popup-body > *:not(.rdc-mini-cart-products):not(.rdc-mini-cart-product):not(#mm-minicart-promo-card):not(#mm-reccart-promo-card):not(.rdc-recomendation-combine) {
          order: 4 !important;
        }
  ...
  // Obter o objeto de dados do cesto de compras diretamente a partir do Angular Scope do $rootScope
    ...
    if (typeof angular !== 'undefined') {
      try {
        ...
        if (injector) {
          ...
          if (rootScope && rootScope.userInfo && rootScope.userInfo.cart) {
            return rootScope.userInfo.cart;
          }
        }
      } catch (e) {
        // Silencioso
  ...
  // Extrator super robusto de IDs a partir de caminhos e parâmetros URL do BZR (para o fallback DOM)
  ...
  // Obter IDs únicos de todos os produtos que estão no carrinho (Angular -> Fallback DOM)
    ...
    if (cart && cart.items && Array.isArray(cart.items)) {
      ...
      cart.items.forEach(item => {
        if (item.product && item.product.id) {
          ids.push(String(item.product.id));
        }
      });
      return ids;
    }
  ...
  // Obter contagem de Havaianas sem contar em duplicado (Angular -> Fallback DOM)
  function getHavaianasCount() {
    const cart = getCartData();
    if (cart && cart.items && Array.isArray(cart.items)) {
      let count = 0;
      cart.items.forEach(item => {
        const brand = item.product?.item_key1_name || '';
    ...
    // FALLBACK DOM (Se o Angular ainda não carregou)
  ...
  // ATUALIZA PROGRESSO EM QUALQUER POPUP E ATUALIZA SUGESTÕES DE FORMA SILENCIOSA E DINEAMICA
  function updatePromoCardProgress($existing) {
    ...
    // Se um dos produtos atualmente sugeridos foi adicionado ao carrinho, baralha e atualiza o DOM interno do card silenciosamente
    const cartProductIds = getCartProductIds();
    ...
    if (hasSuggestedInCart && !isUnlocked) {
      ...
      const $grid = $existing.find('.mm-products-grid');
      if ($grid.length > 0) {
        $grid.html(gridHtml);
        $grid[0].scrollLeft = 0;
      }
      
      const $dots = $existing.find('.mm-dots');
      if ($dots.length > 0) {
        $dots.html('<div class="mm-dot active" data-index="0"></div><div class="mm-dot" data-index="1"></div><div class="mm-dot" data-index="2"></div>');
      }
    }
  ...
  function injectPromoCard($popup, cardId) {
    const $body = $popup.find('.rdc-wrapper-popup-body');
    if ($body.length === 0) return;

    // Se já existe, atualiza progresso se o count mudou e regressa
    const $existing = $popup.find(cardId);
    if ($existing.length > 0) {
      const currentCount = getHavaianasCount();
    ...
    // Injeta por append estável no corpo do popup (ordenado visualmente por Flex CSS)
    $body.append(cardHtml);
  }

  // CORE ENGINE: Polling Leve a cada 400ms (100% livre de MutationObservers e loops infinitos de processamento)
  setInterval(function () {
    // 1. Checkout Page Check
    if (window.location.href.includes('checkout')) {
      const $checkoutCard = $('#mm-promo-card');
      if ($checkoutCard.length === 0) {
        shuffledProducts = getUpdatedSuggestions();
  ...
  // Inicializador básico para verificar reabertura do cesto se aplicável
  ...
  // Pre-load de sugestões iniciais
  shuffledProducts = getUpdatedSuggestions();
